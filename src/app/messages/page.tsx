  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Search based on the selected tab (students or teachers)
        ...
        if (response.ok) {
          ...
          if (groupSearchType === 'teachers') {
            const teacherList: ChatUser[] = results
              .filter((t: any) => t.id !== currentUser.id) // Exclude current user
              .map((t: any) => ({
                id: t.id,
                name: t.name,
            ...
            const studentList: ChatUser[] = results
              .filter((s: any) => s.id !== currentUser.id) // Exclude current user
              .map((s: any) => ({
                id: s.id,
                name: s.name,
